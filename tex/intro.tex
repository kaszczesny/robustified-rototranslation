...
GPS applications struggle during rapid changes of motion, e.g.~while sharp turns are made. Yet driver needs feedback from the positioning system particularly at such moments.
...
This work performs in-depth analysis of a chosen state-of-the-art VO algorithm and proposes some improvements that are especially suited for road scenarios.
...
Tests show that algorithm converges quickly. Some challenges of urban scenarios were not solved, but ways for improvement were suggested.


%Słowa kluczowe – dla porządku i ułatwienia sklasyfikowania naszej pracy opisujemy ją słowami kluczowymi.
\textbf{Keywords}~\==~visual odometry, egomotion, motion estimation, edge detection
