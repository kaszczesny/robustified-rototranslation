...
todo: nieprawda, nie ma APENDIKSOW
...
%?
%“Moravec’s work at Stanford in the 1970s (Moravec, 1980).” TAK

\cite{fast_monocular} \cite{costeira1998multibody} - outlier removal

%Bayes trochę onieśmiela, ale może coś się przyda (do ew. przejrzenia).

\cite{spatiotemporal} - good for full SLAM

%Ulepszenie (?) do GoodFeaturesToTrack
%Nie, raczej alternatywa. Nie zdążymy zaimplementować, a szkoda, bo dzięki informacji czasoprzestrzennej jest to system inteligentny, ale też wydajny

\cite{a_kalman} - By having a camera mounted on the same height and knowing the angle of said camera relative to the ground (pointed slightly down), scale can be estimated.

%Użyto filtru Kalmana do estymacji przepływów optycznych – wymagane pokrycie kolejnych obrazów to tylko 20%; maksymalna wykrywalna szybkość: kilka m/s. KF pozwala przewidzieć transformację obrazu, co redukuje feature space area.

\cite{monoslam} - SLAM using only camera, new approach (camera is not on a robot, but independent)

%Nie tworzy calej mapy, tylko zostawia garsc punktow o bardzo dobrej jakosci wg ktorych mozna sie orientowac (jak napotka drugi raz to samo, to wie ze tu byl i moze poprawic dryf skali)
...
%Metody z dopasowywaniem ficzer pointów to metody indirect, poza tym są metody direct - taką przedstawiono w tym artykule. Jest: gęsta (wykorzystuje wszystkie piksele - przez to pewnie odpada na komórce)

\cite{semi_dense} - semi-dense method

%“In the last years, dense approaches to these challenges have become increasingly popular: Instead of operating solely on visual feature positions, they reconstruct and track on the whole image using a surface-based map and thereby are fundamentally different from feature-based approaches. Yet, these methods are to date either not real-time capable on standard CPU“

\cite{a_stereo_visual} - comparison of feature detectors, consistency check

%Dobry pomysł do feature matchingu: mutual consistency check - feature(t) pasuje najlepiej do feature(t-1), ale również na odwrót
%“To reduce drift of estimation, Bundle Adjustment is a commonly used approach, which optimizes camera poses and the position of features over the past frames. However, the computation cost increases rapidly with the number of frames”
