...
Explanation of the algorthim given in Section~\ref{sec:rebvo_outline} is valid not only for \cite{jose2015realtime}, but also for the discussed implementation. Briefly summarizing:
\begin{enumerate}
	\itemsep-0em 
...
\subsubsection*{Second test -- DoG sign balance}
\label{edge_second}
...
\begin{figure}[ht]
	\centering\includegraphics[width=0.75\linewidth]{img/figures/zeroc2.png}
...
Depth can not be simply copied to current Keylines from corresponding previous ones, as camera has moved between frames, changing distance from the origin to 3D points. An EKF (Extended Kalman Filter) is separately used for each Keyline in estimate current depths; depth uncertainty is reestimated as well. Uncertainty of estimated translation and rotation, obtained in Section~\ref{sec:energyminim}, is taken into account. Prediction and correction equations for $\rho$ are (\ref{eq:kalmanpred}) and (\ref{eq:kalmancor}), respectively

\begin{equation}
 F(\rho_{p}, \vec{v}) = \left( \frac{1}{\frac{1}{\rho_{p}} + v_z} \right) N(1,Q_{rel}) + N(0,Q_{abs})
\label{eq:kalmanpred}
\end{equation}
where:
\begin{eqwhere}[2cm]
	\item[$F$] prediction function,
	\item[$N(\mu, \sigma)$] normal-distributed random variable with mean $\mu$ and standard deviation $\sigma$, 
	\item[$Q_{rel}$] multiplicative noise standard deviation (constant),
	\item[$Q_{abs}$] additive noise standard deviation (constant).
\end{eqwhere}


\begin{equation}
H(\rho_{c}, \vec{v}) = \left( \left( f \left[ v_x,\ \ v_y \right] ^T - h_{c} v_z \right )  \bullet \frac{\vec{g_p}}{\norm{\vec{g_p}}} \right) \rho_{c} + N(0,1)
\label{eq:kalmancor}
\end{equation}
where:
\begin{eqwhere}[2cm]
	\item[$H$] correction function.
\end{eqwhere}

Depth has been constrained in \cite{jose2015realtime} between $\frac{1}{20}$ and $1000$. However, it has been observed that Keylines too close to the camera can disturb the algorithm convergence. Much better results were obtained when minimal allowed depth was raised to $1$. These values are relativ to initial depth in the first frame (mean value of the used distribution) and to other algorithm parameters.

% todo; @kwant: only missing and not finished part of Chapter 2 is this section
 % kalman fiter
...
\begin{itemize}
	\item Haar feature-based cascade classifier combined with ORB (Oriented FAST and Rotated BRIEF) descriptor \cite{opencv},
