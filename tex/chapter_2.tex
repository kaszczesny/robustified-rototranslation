...
Another parameter is window size -- it defines pixel neighborhood that will take part in later calculations of edge position. For window size $w$, $(2w+1)^2$ immediate neighbors will be considered, including the center pixel itself. For sake of this thesis, value $w = 2$ was used. Edge detection procedure is performed for every pixel that lies at least $w$ pixels from image border, so that neighborhood does not need to be extrapolated. The procedure consists of five subsequent tests. If a pixel passes all of them, it is considered to be an edge (and additionally its subpixel position and edge gradient ale calculated along the way).

Influence of these tests on final result is depicted for an exemplary frame in Fig.~\ref{fig:edgeprob}. Color map explanation:
\begin{itemize}
	\itemsep0em
	\item dark blue (0 in the color map) -- neighborhood outside image,
	\item blue (1 in the color map) -- pixel rejected by the first derivative norm test,
	\item light blue (2 in the color map) -- ,
	\item cyan (3 in the color map) -- ,
	\item green (4 in the color map) -- ,
	\item orange (5 in the color map) -- ,
	\item red (6 in the color map) -- reserved for debug purposes,
	\item dark red (7 in the color map) -- pixel identified as Keyline.
\end{itemize}
