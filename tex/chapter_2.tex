...
Some \textbf{thought was given to more elaborate initialization schemes}. For example, todo cite uses feature points during initialization in order to quicker determine scene geometry. Feature point descriptors, such as SIFT \cite{sift}, are well suited for this task. Their main disadvantage is their complexity. 
...
Levenberg-Marquardt algorithm is prone to falling to local minima. Generally, global minimization of arbitrary function is a challenging problem with no infallible solution. There is a~plethora of heuristic methods: evolutionary algorithms, simulated annealing, physical simulations, etc. However, they are unsuitable for real-time applications \cite{szeliski}.
