...
\begin{enumerate}
	...
	\item Due to reasons mentioned in Section~\ref{sec:mapping}, in order to find matches between previous and current Keylines, additional matching has to be executed~\==~Section~\ref{sec:directed}. Depth information is propagated from previous frame to current, smoothed out (Section~\ref{sec:regul}) and updated to take into account the motion that camera has underwent (Section~\ref{sec:kalman}).
\end{enumerate}
