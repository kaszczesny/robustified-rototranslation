...
todo Czy wykrywanie krawedzi jest robust na zmiany jasnosci;
...
%
%  motion consistency
Searching starts at a point that is estimated to be a most possible match. The procedure is performed checking pixels closer to segment ends, in an alternating manner. If an inspected pixel a previous Keyline, it is considered a possible match, that still needs to be validated. Firstly, $\vec{g_p}$ and $\vec{g_r}$ are compared, analogously to the third keyline matching test of the minimizer (see Section~\ref{sec:minim_match}). Secondly, potential match needs to conform to the motion model.

todo: rownanie, costam costam, ogarnialismy  ----- so much R.I.P.
...
This doesn't remedy the fundamental problem of scale ambiguity in the monocular systems. To fix this problem real-life dimensions of an object present in starting frames are needed. Some objects having known, well defined shape and size that are ever-present in urban environments are traffic signs and license plates. Solution for identifying those objects are:
\begin{itemize}
	\item Haar feature-based cascade classifier, then ORB
	\item Neural Networks
	\item SVN
	\item morphological operations
	\item detecting edges of an object and applying Hough transform on the edges
	\item Robust license plate detection using image saliency /citeme/
\end{itemize}

This however means additional, heavy computing cost and would not fit the designated platform and moreover no simple or easy to implement out-of-the-box implementation was found thus moving this problem out of the scope of this thesis.

% ----------------------
\subsection{Artificial data tests}

Along with data from previously mentioned datasets, some tests on generated data were performed, mainly for EdgeFinder part of the algorithm. That way we could see how difficult edges (hard angles (i.e. T,Y,L,+ shaped) or circles) are thinned, joined, fragmented and how gradient on such edges will behave.
2D transformations are a special case of 3D ones, so it was also tested how those will be recovered, however lack of depth data generated a lot of false positives. Rendering a sequence in a computer 3D environment was also considered, but due to antialiasing, poor lighting techniques that are processor-friendly, texturing(bump-mapping) and raytracing often used by those types of environments they could falsify obtained information. Because of that datasets with well-defined ground truth were chosen instead.
