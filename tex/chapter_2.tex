...
\begin{figure}[ht]
	...
	\end{subfigure}
	\caption{\label{fig:bucket} Edge detection in 4 consecutive KITTI \cite{kitti} frames under lighting conditions changes~\==~input images}
\end{figure}
...
Then DoG values are approximated by a plane using linear regression -- Eq.~\ref{eq:regress} is solved for~$\theta$. Pseudo-inverse of $\tmmathbf{A}$, needed to solve it, needs to be computed only once for the whole algorithm. Zero-crossing of the plane can be determined algebraically, resulting in Eq.~\ref{eq:zerocross}. These closed-form formulas are the main reason for DoG not being computationally demanding, as curve-fitting approaches are usually iterative.
