...
Sometimes frames did not contain enough Keylines and system had to be reinitialized. In case of TUM fr3\_teddy sequence there are 4 cases of reinitialization visible in Fig.~\ref{fig:teddyfull}: frames 271 \& 272, 280, 386 \& 387 and 394. System recovered from three out of four of them, but the third was fatal in a sense that scale and starting rototranslation were lost, and in extension that trajectory as a whole can not be well fitted with ground truth (Fig.~\ref{fig:teddynok}). However, partitioned sections can and are fitted quite well, shown on Fig.~\ref{fig:teddy1} and Fig.~\ref{fig:teddy2} for the first part and the second one, respectively. Such points could be potentially detected in post-processing, as they are a sharp spots of a otherwise smooth 3D curve.
...
Places where rototranslation is lost due to reinitialization prevent usage of SLAM loop closure methods, as the 3D curves are very far from closing. For instance, in Fig.~\ref{fig:loop}, an expected loop and obtained trajectory are compared.
...
todo wywiniecie + obrazek glebi + ze dalej leci dobrze
